 ...
 */
  ...
  getPacketByteLen(str: string) {
    let hexLen = str.substring(8, 12);
    return parseInt(hexLen, 16);
  }

  getPacketByteFramNum(str: string) {
    let hexLen = str.substring(12, 18);
    return parseInt(hexLen, 16);
  }
  ...
  addMsgIdProtocol(msgId:string,data: string) {
    return BLE_HEAD +msgId+ "00"+this.getHexByteLength(data) + data + BLE_TRAIL;
  }
